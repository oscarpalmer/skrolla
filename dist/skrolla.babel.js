"use strict";

(function (scope) {
  var doc = scope.document,
      ceiling = Math.ceil,
      powerOf = Math.pow,
      objectPrototype = Object.prototype,
      options = {
    callback: null,
    offset: 0
  },
      addListeners = function addListeners() {
    var elements = doc.querySelectorAll("[data-skrolla]");

    Array.prototype.forEach.call(elements, function (element) {
      element.addEventListener("click", onClick);
    });
  },
      easing = function easing(time) {
    if ((time *= 2) < 1) {
      return .5 * powerOf(time, 3);
    }

    return .5 * ((time -= 2) * powerOf(time, 2) + 2);
  },
      getDuration = function getDuration(distance) {
    return ceiling(distance < 1 ? -distance : distance);
  },
      getElement = function getElement(target) {
    return doc.getElementById(target) || doc.body;
  },
      getOffset = function getOffset(element) {
    return element.getBoundingClientRect().top - options.offset;
  },
      getTarget = function getTarget(target) {
    if (target.nodeType) {
      return target;
    }

    return typeof target === "string" ? getElement(target) : doc.body;
  },
      onClick = function onClick(event) {
    event.preventDefault();

    var target = event.target,
        targetId = target.getAttribute("data-skrolla");


    skrollTo(targetId, target);
  },
      setOptions = function setOptions(object) {
    if (objectPrototype.toString.call(object) !== "[object Object]") {
      return;
    }

    for (var property in object) {
      if (objectPrototype.hasOwnProperty.call(object, property)) {
        options[property] = object[property];
      }
    }

    if (typeof options.callback === "function") {
      skrollaCallback = true;
    }
  },
      skrollTo = function skrollTo(target, origin) {
    var page = scope.pageYOffset,
        element = getTarget(target),
        offset = getOffset(element),
        duration = getDuration(offset),
        loop = function loop(time) {
      if (start === null) {
        start = time;
      }

      var elapsed = time - start,
          distance = offset * easing(elapsed / duration);

      scope.scrollTo(0, page + distance);

      if (elapsed < duration) {
        scope.requestAnimationFrame(loop);
      } else {
        scope.scrollTo(0, page + offset);
        if (skrollaCallback) {
          options.callback.call(element, element, origin);
        }
      }
    };

    var start = null;

    scope.requestAnimationFrame(loop);
  };

  var skrollaActive = false,
      skrollaCallback = false;

  scope.skrolla = function (userOptions) {
    if (skrollaActive) {
      return;
    }

    skrollaActive = true;

    setOptions(userOptions);

    addListeners();
  };

  scope.skrolla.to = function (target) {
    skrollTo(target, scope);
  };
})(window);